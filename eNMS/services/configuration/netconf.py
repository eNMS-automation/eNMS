...
from eNMS.forms.automation import ServiceForm, ConnectionForm
            ...
            try:
              ...
              if run.xml_conversion:
                try:
                  ...
                  result = {
                      "success": False,
                      "result": f'xmltodict failed to parse response: {repr(kerr)}',
                  }
              result = {
                    "success": True,
                    "result": config,
              ...
              result = {
                    "success": False,
                    "result": f'Invalid result from ncclient manager get_config: {repr(exc)}',
              }
            ...
            try:
              ...
              if run.xml_conversion:
                try:
                  ...
                  result = {
                          "success": False,
                          "result": f'xmltodict failed to parse: {repr(kerr)}',
                  }
              result = {
                      "success": True,
                      "result": get_filter
...
#Push Config
          if run.nc_type == "push_config":
            ...
            try:
              ...
              if run.commit_conf == True: m.commit()
              if run.xml_conversion:
                try:
              ...
              result = {
                        "success": False,
                        "result": f'Invalid result from ncclient manager edit_config: {repr(exc)}',
              }
#Copy config
          if run.nc_type == "copy_config":
            ...
            try:
              ...
              result = {
                        "success": False,
                        "result": f'Invalid result from ncclient manager config copy: {repr(exc)}',
              }
#Remote Procedure Call
          if run.nc_type == "rpc":
            try:
              ...
              result = {
                        "success": False,
                        "result": f'Invalid result from ncclient manager rpc: {repr(exc)}',
              }
#Unlock target
          if run.unlock == True:
            try:
...
class NetconfForm(ConnectionForm):
  ...
  xml_filter = StringField(label='XML Filter', widget=TextArea(), render_kw={"rows": 5}, substitution=True)
  target = SelectField(
    choices=(
            ("running", "Running"),
            ("candidate", "Candidate"),
  ...
  source_url = StringField(label='Copy source URL', widget=TextArea(), render_kw={"rows": 1}, substitution=True)
  ...
  destination_url = StringField(label='Copy destination URL', widget=TextArea(), render_kw={"rows": 1}, substitution=True)
  ...
  groups = {
      "NETCONF Parameters": {"commands": ["nc_type","xml_filter","target","copy_source","source_url","copy_destination","destination_url","default_operation","test_option","error_option","lock","unlock","commit_conf","xml_conversion"], "default": "expanded"},
  ...
  input_data = HiddenField(
    "",
    default=dumps({
      # add all fields to the field list except for "nc_type" which will drive the selection
      # every field in this list will be hidden unless it is contained in one of the netconf type
      # entries below.
      "fields": ["xml_filter","target","copy_source","source_url","copy_destination","destination_url","default_operation","test_option","error_option","lock","unlock","commit_conf","xml_conversion"],
      
      # add all the different netconf commands as keys with the list of fields to be shown 
      ...
      "netconf_type": {
        ...
        "push_config": [
          ...
          "error_option", "lock", "unlock", "commit_conf","xml_conversion"
        ],
        "copy_config": ["copy_source", "source_url","copy_destination","destination_url","commit_conf","xml_conversion"],
