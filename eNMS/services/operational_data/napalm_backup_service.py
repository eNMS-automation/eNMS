    ...
    def job(self, run, payload, device):
        ...
        try:
            ...
            for data_type in ("configuration", "operational_data"):
                value = run.sub(getattr(self, data_type), locals())
                if data_type == "configuration":
                    for getter in run.configuration:
                        try:
                            result = getattr(napalm_connection, getter)()
                            for r in self.replacements:
                                result = sub(r["pattern"], r["replace_with"], str(result), flags=M)
                        except Exception as e:
                            result = f"{getter} failed because of {e}"
                else:
                    for getter in run.operational_data:
                        try:
                            result = getattr(napalm_connection, getter)()
                            for r in self.replacements:
                                result = sub(r["pattern"], r["replace_with"], str(result), flags=M)
