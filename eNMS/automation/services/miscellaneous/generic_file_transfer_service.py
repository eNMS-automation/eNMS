...
class GenericFileTransferService(Service):
    ...
    def job(self, device, payload):
        ...
        try:
            ...
            ssh.connect(device.ip_address, username=device.username, password=device.password)
            if filetransfer_type == 'sftp':
                sftp = ssh.open_sftp()
                if filetransfer_direction == 'put':
        ...
        return {'success': True, 'result': 'File %s transferred successfully' % source_file}
